...
class HeadFrame(customtkinter.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.entry_employer = customtkinter.CTkEntry(
            self, placeholder_text="Employer", width=400
        )
        self.entry_employer.grid(row=0, column=0, padx=15, pady=15, sticky="ns")

        self.entry_headoffice = customtkinter.CTkEntry(
            self, placeholder_text="Head Office"
        )
        self.entry_headoffice.grid(row=0, column=1, padx=15, pady=15, sticky="ns")

        self.entry_localoffice = customtkinter.CTkEntry(
            self, placeholder_text="Local Office"
        )
        self.entry_localoffice.grid(row=0, column=2, padx=15, pady=15, sticky="ns")

        self.entry_employee = customtkinter.CTkEntry(
            self, placeholder_text="Name Employee", width=400
        )
        self.entry_employee.grid(row=1, column=0, padx=15, pady=15, sticky="ns")

        self.optionmenu_month = customtkinter.CTkOptionMenu(
            self, values=months, command=self.optionmenu_month_callback
        )
        self.optionmenu_month.set(calendar.month_name[month])
        self.optionmenu_month.grid(row=1, column=1, padx=15, pady=15, sticky="ns")

        self.optionmenu_year = customtkinter.CTkOptionMenu(
            self, values=years, command=self.optionmenu_year_callback
        )
    ...
    def update_frame_by_date(self):
        for widget in self.master.scrollable_checkbox_frame.winfo_children():
            widget.destroy()
        self.master.scrollable_checkbox_frame.grid(
            row=1, column=0, padx=15, pady=15, sticky="ns"
        )
        self.master.scrollable_checkbox_frame.update_list(

...
class ScrollableCheckBoxFrame(customtkinter.CTkScrollableFrame):
    ...
    def build_table_head(self):
        ...
        for title in ["DAY", "DAYWEEK", "1st. Period","2nd. Period"]:
            label=customtkinter.CTkLabel(self, text=title, compound="left", padx=15, anchor="w")
            label.grid(row=0, column=count, padx=(10, 10), pady=(10, 10))
            self.checkbox_list.append(label)
            count=count+1
        
    def add_item(self, item):
        label_num=customtkinter.CTkLabel(self, text=item[0], compound="left", anchor="w")
        label_name=customtkinter.CTkLabel(self, text=item[1], compound="left", padx=15, anchor="w")
        ...
        label_num.grid(row=len(self.checkbox_list), column=0, padx=(10, 10), pady=(10, 10))
        label_name.grid(row=len(self.checkbox_list), column=1, padx=(10, 10), pady=(10, 10))
        checkbox.grid(row=len(self.checkbox_list), column=2, padx=(10, 10), pady=(10, 10))
        checkbox2.grid(row=len(self.checkbox_list), column=3, padx=(10, 10), pady=(10, 10))
        checkbox.select()
        checkbox2.select()

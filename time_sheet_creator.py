...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.scrollable_checkbox_frame = ScrollableCheckBoxFrame(master=self, width=800, height=400)
        self.scrollable_checkbox_frame.grid(row=2, column=0, padx=15, pady=5, sticky="ns")
        self.scrollable_checkbox_frame.update_list(utils.get_list_of_days_weeks(month, year))

        self.feedback_frame = FeedbackFrame(master=self, fg_color="transparent")
        self.feedback_frame.grid(row=3, column=0, padx=0, pady=0, sticky="ns")
        
        self.button = customtkinter.CTkButton(self, text="GENERATE", command=self.button_event)
        self.button.grid(row=4, column=0, padx=15, pady=15, sticky="ns")
        
    def button_event(self):
        list_items=self.scrollable_checkbox_frame.get_checked_list()
        file_name=""   
        try:            
            file_name=utils.generate_pdf(list_items, self.head_frame.get_head_elements())
    ...
    def display_dialog_error(self, message):
        ...
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            self.toplevel_window=DialogError(self)  # create window if its None or destroyed

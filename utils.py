...
package_path = pathlib.Path(
    str(pathlib.Path.cwd()) + "/translations/translate-en_pt-1_0.argosmodel"
)
...
def generate_pdf(destination, list_items, headers):
    ...
    with pdf.table(text_align="CENTER", borders_layout="NONE") as table:
        row = table.row()
        row.cell(
            text="FOLHA DE REGISTO DE HORAS TRABALHADAS",
            style=FontFace(size_pt=DOC_HEADER_FONT_SIZE),
        )

    pdf.ln()

    with pdf.table(
        text_align="LEFT", num_heading_rows=2, col_widths=(11, 49, 6, 19, 6, 19)
    ) as table:
        row = table.row()
    ...
    with pdf.table(
        text_align="CENTER", num_heading_rows=2, col_widths=(10, 10, 10, 10, 10, 25, 25)
    ) as table:
        heading = table.row()
    ...
    pdf.output(name=destination + "/" + file_name, dest="F")
    return file_name
